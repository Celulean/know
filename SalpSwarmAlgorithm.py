...
address = './salptest.xlsx'
df = pd.read_excel(address)

sarima = df["SARIMA"].values
randomf = df['RANDOMF'].values
lstm = df["LSTM"].values
#mowoa = df['MOWOA'].values
actual = df["Actual power"].values
#print(mopso.shape)
...
# loss_function = ["mape", 'mse'] # 可选"mse", "mae", "mape", "std" 必须为list形式
# opt_result = SalpSwarmAlgorithm(swarm_size, min_value_weights, max_value_weights, iterations, loss_function,  lstm, randomf, sarima, actual)  # 实例化优化方法
